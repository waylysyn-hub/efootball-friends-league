...
export function errorMessage(error, fallback = 'تعذّر تنفيذ العملية. حاول مجددًا؛ وإذا تكرر الخطأ حدّث الصفحة.') {
  ...
  const known = {
    ...
    EFL_EVENING_CHANGED: 'هذا الطلب محفوظ باختيارات مختلفة. اضغط تحديث لعرض القرعة المحفوظة.',
    EFL_EVENING_CLOSED: 'لا يمكن إعادة فتح سهرة منتهية. ابدأ سهرة جديدة.',
    EFL_EVENING_MISSING: 'السهرة لم تعد متاحة. اضغط تحديث وحاول مجددًا.',
    EFL_INVALID_MATCH: 'بيانات المباراة غير مكتملة. اختر لاعبين مختلفين وتحقق من النتيجة والتاريخ والموسم.',
    EFL_GOAL_COUNT: 'عدد الأهداف لا يطابق النتيجة. أكمل تفاصيل كل هدف أو اختر حفظ النتيجة فقط.',
