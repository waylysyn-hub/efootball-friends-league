/* =====================================================
   League security runtime
   Loaded after script.js to replace legacy browser-only auth paths while
   preserving the existing UI and league feature set.
   ===================================================== */
...
fetchAllData = async function () {
  ...
  // Restore a real Supabase session before loading protected league data.
  try {
  ...
  const [players, seasons, matches, questions, answers, matchStats, goalEvents] = await Promise.all([
    // Never fetch password/auth_user_id into the browser.
...
// Standings and achievements are database-derived. Client writes are disabled.
persistStandings = async function () {};
persistAchievements = async function () {};
syncDerivedData = async function () {};

// Restore the last selected name for convenience only; it is never trusted as auth.
document.addEventListener('DOMContentLoaded', () => {
  try {
    const remembered = localStorage.getItem('efl_last_user');
    const select = document.getElementById('loginUsername');
    if (remembered && select && [...select.options].some((o) => o.value === remembered)) {
