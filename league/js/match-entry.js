...
import { esc, navigateTo, renderPage, showConfirm, showError, showToast } from './ui.js';
import { getActiveSeason, populateSeasonDropdowns } from './seasons.js';
import { getPlayers, populateMatchPlayerDropdowns } from './profiles.js';
import { collectGoalEventsFromForm, countGoalsByOwner, getMatchGoalEvents, renderGoalEventsForm, updateGoalEventsUI, validateGoalEvents } from './goal-events.js';
...
const validPair = data => getPlayers().includes(data.Player1) && getPlayers().includes(data.Player2) && data.Player1 !== data.Player2;
...
function filterOpponents(prefix) {
  for (const [side, other] of [[1, 2], [2, 1]]) {
    ...
    for (const option of el(prefix + 'Player' + side).options) {
      option.hidden = option.disabled = !!option.value && option.value === opponent;
    }
  }
}
...
function syncRows(prefix, events, previous, onCancel = null) {
  ...
  if (discarded) {
    ...
    showConfirm('تقليل عدد الأهداف', 'سيؤدي تقليل النتيجة إلى حذف تفاصيل الأهداف الزائدة. هل تريد المتابعة؟', () => {
    ...
    }, () => { draft.pending = false; onCancel?.(); paint(prefix); });
  } else {
...
export function onMatchEntryChange(prefix = 'match') {
  ...
  clearErrors(prefix);
  if (data.Player1 && data.Player1 === data.Player2) fieldError(prefix + 'Player2', 'اختر فريقًا مختلفًا عن الفريق الأول.');
...
function validate(prefix) {
  ...
  for (const side of [1, 2]) {
    if (!getPlayers().includes(data['Player' + side])) fieldError(prefix + 'Player' + side, 'اختر الفريق من قائمة لاعبي الدوري.');
    if (!validScore(data['Goals' + side])) fieldError(prefix + 'Goals' + side, 'أدخل عددًا صحيحًا من 0 إلى 99، دون كسور.');
  }
  if (data.Player1 && data.Player1 === data.Player2) fieldError(prefix + 'Player2', 'لا يمكن أن يلعب الفريق ضد نفسه. اختر فريقين مختلفين.');
  ...
  if (detailed) {
    ...
    rows.forEach((row, i) => {
      ...
      if (minute.value !== '' && (!Number.isInteger(Number(minute.value)) || Number(minute.value) < 1 || Number(minute.value) > 120)) fieldError(minute.id, 'اترك الدقيقة فارغة أو أدخل عددًا صحيحًا من 1 إلى 120.');
      if (events[i].assist && events[i].scorer.toLowerCase() === events[i].assist.toLowerCase()) fieldError(row.querySelector('.ge-assist').id, 'اختر زميلًا أو «بدون أسيست».');
    });
  }
  ...
  return { data, events: events.map(({ owner, scorer, assist, minute }, sortOrder) => ({ owner, scorer, assist, minute, sortOrder })) };
}
...
export function saveMatchForm(editing) {
  ...
  el('matchReviewSummary').innerHTML = `<p class="entry-review-score" dir="rtl">${scoreHTML(data)}</p>
    <dl class="entry-review-list"><div><dt>الموسم</dt><dd>${esc(displaySeason(season.name))}</dd></div><div><dt>التاريخ</dt><dd><bdi>${esc(data.Date)}</bdi></dd></div>
    <div><dt>عدد الأهداف</dt><dd>${Number(data.Goals1) + Number(data.Goals2)}</dd></div><div><dt>تفاصيل أهداف مكتملة</dt><dd>${events.length}</dd></div><div><dt>الأسيست المسجّل</dt><dd>${events.filter(row => row.assist).length}</dd></div></dl>
...
export async function confirmMatchSave() {
  ...
  return withBusy(editing ? 'edit-match' : 'save-match', el('confirmMatchSaveButton'), async () => {
    el('matchReviewError').classList.add('hidden');
    try {
      const { error } = await sb.rpc('save_league_match', { match_data: match, goal_events: events });
      ...
      const saved = { ...match, season: match.season_id };
      state.db.matches = [...state.db.matches.filter(row => row.id !== id), saved];
      state.db.goalEvents = [...state.db.goalEvents.filter(row => row.matchId !== id), ...events.map((row, i) => ({ ...row, id: `${id}-${i}`, matchId: id }))];
